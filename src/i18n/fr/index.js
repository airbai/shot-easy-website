import nav from './nav';
import editor from './editor';
import beautifier from './beautifier';
import rounded from './rounded';
import remover from './remover';
import compressor from './compressor';
import screenshot from './screenshot';

export default {
    title: 'éditeur de photos en ligne gratuit',
    description:
        "Éditez des photos en ligne gratuitement, redimensionnez et appliquez des filtres à n'importe quelle photo, éditez des photos dans le navigateur, convertissez des images en jpg/png/jpeg/webp, capturez facilement des zones ou des pages entières",
    keywords:
        "Pic Fiit, easy shot, capture d'écran, éditer photo, convertisseur de photos, convertisseur d'images, éditeur en ligne, changer le format d'image en ligne, convertir une image en jpg, jpg en webp, jpg en png",
    privacy: 'Confidentialité',
    terms: 'Conditions',
    blog: 'Blog',
    nav,
    editor,
    beautifier,
    rounded,
    remover,
    compressor,
    screenshot
};
